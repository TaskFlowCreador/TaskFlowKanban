...
// ===================== TARJETA DE TAREA =====================
const Task = React.memo(function Task({ task, colId, index, onStartDrag, onDelete, onAdvance, onEdit, dragging, animClass }) {
  ...
  return (
    <div
      className={`task ${task.color}${dragging ? ' dragging' : ''}${animClass ? ' ' + animClass : ''}`}
...
export default function App() {
  ...
  // ---------- Animación: qué tarea está "entrando" o "saliendo" en este instante ----------
  const [enteringIds, setEnteringIds] = useState(() => new Set());
  const [leavingMap, setLeavingMap] = useState(() => new Map()); // taskId -> 'task-leaving-delete' | 'task-leaving-advance'
  const [glowCol, setGlowCol] = useState(null);
  const [pulseCol, setPulseCol] = useState(null);

  const markEntering = (taskId) => {
    setEnteringIds(prev => new Set(prev).add(taskId));
    setTimeout(() => setEnteringIds(prev => { const n = new Set(prev); n.delete(taskId); return n; }), 450);
  };
  const pulseBadge = (colId) => {
    setPulseCol(colId);
    setTimeout(() => setPulseCol(null), 400);
  };
  ...
  const addTask = () => {
    ...
    const newId = uid();
    setBoard(prev => ({ ...prev, todo: [...prev.todo, { id: newId, text, color: selectedColor }] }));
    setInputValue('');
    showToast('add');
    markEntering(newId);
    pulseBadge('todo');
  ...
  const deleteTask = (colId, taskId) => {
    // Primero se anima "saliendo" (encoge y se desvanece), y solo cuando termina la animación
    // se quita de verdad del tablero — así la nota se siente real, no que desaparece de golpe.
    setLeavingMap(prev => new Map(prev).set(taskId, 'task-leaving-delete'));
    showToast('delete');
    setTimeout(() => {
      setBoard(prev => ({ ...prev, [colId]: prev[colId].filter(x => x.id !== taskId) }));
      setLeavingMap(prev => { const n = new Map(prev); n.delete(taskId); return n; });
    }, 380);
  };
  ...
  const advanceTask = (colId, taskId) => {
    ...
    // 1) La tarjeta "vuela" hacia afuera de su columna actual...
    setLeavingMap(prev => new Map(prev).set(taskId, 'task-leaving-advance'));
    showToast('move');

    setTimeout(() => {
      // 2) ...y solo entonces se mueve de verdad en los datos, y "aterriza" con un pop en la nueva columna.
      const task = board[colId].find(x => x.id === taskId);
      setBoard(prev => ({
        ...prev,
        [colId]: prev[colId].filter(x => x.id !== taskId),
        [next]: task ? [...prev[next], task] : prev[next]
      }));
      setLeavingMap(prev => { const n = new Map(prev); n.delete(taskId); return n; });
      pulseBadge(next);
      markEntering(taskId);
      if (next === 'done') {
        celebrate();
        setGlowCol('done');
        setTimeout(() => setGlowCol(null), 1500);
      }
    }, 360);
  };
  ...
  return (
    <>
      ...
      <div className="board" onPointerMove={onDragMove} onPointerUp={endDrag} onPointerCancel={endDrag}>
        {COLS.map(colId => (
          <div className={`column${glowCol === colId ? ' col-done-glow' : ''}`} key={colId}>
            <div className="column-header"><span>{t[colId]}</span><span className={`count-badge${pulseCol === colId ? ' badge-pulse' : ''}`}>{board[colId].length}</span></div>
              ...
              {board[colId].map((task, index) => (
                <div data-task-id={task.id} key={task.id} style={dragTaskId === task.id ? { opacity: 0 } : undefined}>
                  <Task
                    ...
                    animClass={leavingMap.get(task.id) || (enteringIds.has(task.id) ? 'task-entering' : '')}
                  />
                </div>
              ))}
