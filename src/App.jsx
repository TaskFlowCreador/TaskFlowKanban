...
// ===================== TARJETA DE TAREA =====================
const Task = React.memo(function Task({ task, colId, index, onStartDrag, onDelete, onDeleteFinish, onAdvance, onEdit, dragging, deleting }) {
  ...
  return (
    <div
      className={`task ${task.color}${dragging ? ' dragging' : ''}${deleting ? ' task-deleting' : ''}`}
      onPointerDown={(e) => { if (!deleting) onStartDrag(e, colId, index, task.id); }}
      onAnimationEnd={(e) => { if (deleting && e.animationName === 'taskLeaveDelete') onDeleteFinish(colId, task.id); }}
    >
      <span
        ref={spanRef}
        className="task-text"
        contentEditable={!deleting}
...
// ===================== APP PRINCIPAL =====================
// Contador de una columna. Detecta sus propios cambios (comparando con el valor anterior) y hace
// un "pop" con transition — nunca con "animation", precisamente para no repetir el mismo choque
// que causaba el bug de la columna: transition solo interpola hacia un estado, nunca "reinicia".
function CountBadge({ count }) {
  ...
  return (
    <span
      className={`count-badge${popping ? ' badge-pop' : ''}`}
      onTransitionEnd={() => setPopping(false)}
    >{count}</span>
  );
}
...
export default function App() {
  ...
  // ---------- Animación robusta: sin temporizadores adivinados ----------
  // En vez de "calcular" cuánto dura una animación con setTimeout (frágil: si se hace doble clic
  // rápido, o el celular es lento, los tiempos se desincronizan y algo se queda a medias — esa era
  // la causa del bug), se usan dos técnicas nativas del navegador:
  // 1) Las tarjetas nuevas usan una animación CSS que se dispara SOLA cuando React las monta en el
  //    DOM (no hace falta ninguna bandera ni temporizador para "avisarle" que apareció).
  // 2) Al borrar, se espera el evento real onAnimationEnd ("la animación ya terminó de verdad")
  //    antes de quitar la tarjeta de los datos — nunca se puede desincronizar porque es el propio
  //    navegador quien avisa, no una suposición de tiempo.
  const [deletingIds, setDeletingIds] = useState(() => new Set());
  const [doneBounceId, setDoneBounceId] = useState(null);
  ...
  // Paso 1: marcar la tarjeta como "borrándose" (dispara la animación vía CSS).
  const deleteTask = (colId, taskId) => {
    setDeletingIds(prev => new Set(prev).add(taskId));
    showToast('delete');
  };

  // Paso 2: el propio navegador llama esto cuando la animación de salida realmente terminó.
  const finishDelete = (colId, taskId) => {
  ...
  // Mover una tarea ahora es instantáneo en los datos (sin retraso artificial); la sensación de
  // "aterrizaje" viene sola porque React monta la tarjeta como elemento nuevo en la otra columna,
  // y esa columna ya tiene la animación de entrada aplicada por CSS — cero coordinación manual.
  const advanceTask = (colId, taskId) => {
    const order = ['todo', 'doing', 'done'];
    const next = order[order.indexOf(colId) + 1];
    ...
    if (next === 'done') {
      celebrate();
      setDoneBounceId(taskId); // hace "rebotar" la columna Hecho una sola vez, ver más abajo
    }
  };
  ...
  const endDrag = () => {
    ...
    if (finalCol === 'done') { celebrate(); setGlowCol('done'); setTimeout(() => setGlowCol(null), 1500); }
    showToast('move');
  };
  ...
  return (
    <>
      ...
      <div className="board" onPointerMove={onDragMove} onPointerUp={endDrag} onPointerCancel={endDrag}>
        {COLS.map(colId => (
          <div
            className={`column${colId === 'done' && doneBounceId ? ' done-bounce' : ''}`}
            key={colId}
            onTransitionEnd={(e) => { if (e.target === e.currentTarget && e.propertyName === 'transform') setDoneBounceId(null); }}
          >
            <div className="column-header"><span>{t[colId]}</span><CountBadge count={board[colId].length} /></div>
              ...
              {board[colId].map((task, index) => (
                <div data-task-id={task.id} key={task.id} style={dragTaskId === task.id ? { opacity: 0 } : undefined}>
                  <Task
                    ...
                    onStartDrag={startDrag} onDelete={deleteTask} onDeleteFinish={finishDelete}
                    onAdvance={advanceTask} onEdit={editTask}
